...
function dameContenido(url){
  try{
    const r = $.ajax({ url, method: 'GET', async: false, cache: false });
    const txt = r.responseText;
    if (r.status !== 200 || !txt){
      console.error('dameContenido fallo =>', { url, status: r.status, statusText: r.statusText, len: (txt||'').length, preview: (txt||'').substring(0,120) });
    }
    return txt || "";
  }catch(e){
    console.error('dameContenido EXCEPTION', url, e);
    return "";
  }
}
...
function ejecutarAjax(url, data){
  try{
    ...
    console.error("ejecutarAjax error", url, e);
    return "";
  }
}
